...
void ResourceManager::CreateDefaultShader()
{
	...
	vertexShader->Create(L"Default.hlsl", "VS", "vs_5_0");						// CreateVS
	// GPU와 쉐이더랑 연결
	auto inputLayout = make_shared<InputLayout>(_device);
	inputLayout->Create(VertexTextureData::descs, vertexShader->GetBlob());	// CreateInputLayout
	// PixelShader 생성
	auto pixelShader = make_shared<PixelShader>(_device);
	pixelShader->Create(L"Default.hlsl", "PS", "ps_5_0");						// CreatePS
